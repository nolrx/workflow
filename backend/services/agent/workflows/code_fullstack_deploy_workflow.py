...
_PHASE_PROGRESS = {
    "provision": 1, "migrate": 2, "build": 3, "start": 4, "health": 5, "smoke": 6,
    "itest": 7, "done": 7,
}
...
def run_code_fullstack_deploy_workflow(ctx, recorder) -> dict:
    ...
    with recorder.step(
        ...
        result = deploy_service.deploy(
            ...
            on_phase=on_phase, is_cancelled=ctx.is_cancelled, run_id=ctx.run_id,
        )
        ...
        if result.get("repaired") and result.get("repaired_source"):
            ...
            step.add_artifact(
                AgentArtifactType.TEXT, "后端工程源码（部署自动修复版，zip）",
                filename="backend_project_repaired.zip", mime_type="application/zip",
                write_file=True, content_bytes=buffer.getvalue(),
                domain_ref_type="code_backend_project_repaired_zip", domain_ref_id=project_id,
            )
            step.add_artifact(
                AgentArtifactType.JSON, "部署自动修复摘要",
                content_json=result.get("fix_summary") or {},
                filename="deploy_fix_summary.json",
                domain_ref_type="code_deploy_fix_summary", domain_ref_id=project_id,
            )
        ...
        step.set_output(
            ...
            reasoning_summary="有序原子部署:建库 → docker build 后端镜像 → 启动长驻容器 → 健康检查 → 契约冒烟 → 拉取前端调用代码+契约做全面接口测试;发现确定性失败则只改后端自动修复(重建+重启+重测,尽力放行)→ 接入反代。修复版后端源码已发布(下载/同步/重部署优先取修复版)。",
