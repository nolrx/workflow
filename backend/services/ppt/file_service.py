"""
PPT File Service - handles all file operations for PPT Studio
"""
import logging
import shutil
...
def get_ai_provider_for_user(user_id: str) -> Optional[Any]:
    """
    Get AI provider instance from environment variables.

    Uses the unified factory configuration from backend.services.ai.factory.

    Args:
        user_id: User ID (currently unused, reserved for future per-user config)

    Returns:
        AI provider instance or None if not configured
    """
...
class PPTFileService:
    """Service for PPT file management"""
    ...
    def _get_project_dir(self, project_id: str) -> Path:
        ...
        project_dir = self.upload_folder / "ppt" / project_id
    ...
    def _get_user_templates_dir(self) -> Path:
        ...
        templates_dir = self.upload_folder / "ppt" / "user-templates"
        templates_dir.mkdir(exist_ok=True, parents=True)
        return templates_dir

    def _get_global_materials_dir(self) -> Path:
        """Get global materials directory"""
        materials_dir = self.upload_folder / "ppt" / "materials"
        materials_dir.mkdir(exist_ok=True, parents=True)
        return materials_dir
    ...
    def save_material_image(self, image: Image.Image, project_id: Optional[str],
        ...
        if project_id is None:
            materials_dir = self._get_global_materials_dir()
        else:
    ...
    def get_file_url(self, project_id: Optional[str], file_type: str, filename: str) -> str:
        ...
        if project_id is None:
            # Global materials
            return f"/api/ppt/files/materials/{filename}"
        return f"/api/ppt/files/{project_id}/{file_type}/{filename}"
    ...
    def get_template_path(self, project_id: str) -> Optional[str]:
        ...
        from backend.models.ppt import PPTProject
        ...
        if project and project.template_image_path:
            # template_image_path is relative path, convert to absolute
            template_path = self.upload_folder / project.template_image_path
            if template_path.exists() and template_path.is_file():
                return str(template_path)

        # Fallback to directory search for backward compatibility
        template_dir = self._get_template_dir(project_id)
        if template_dir.exists():
            # Sort by modification time, return the newest template file
            template_files = [
